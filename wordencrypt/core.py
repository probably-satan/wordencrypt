...
# Kept for backward compatibility; insert_homoglyphs now uses HOMOGLYPH_POOLS.
HOMOGLYPHS = {
    "a": "\u0430",  # Cyrillic а
    "e": "\u0435",  # Cyrillic е
    "o": "\u043e",  # Cyrillic о
...
# Mathematical italic lowercase: U+1D44E + offset, except h → U+210E
def _math_lower(base: int, letter: str, exceptions: dict | None = None) -> str:
    if exceptions and letter in exceptions:
        return exceptions[letter]
    c = chr(base + ord(letter) - ord("a"))
    import unicodedata
    return c if unicodedata.name(c, "") else ""
...
_FRAKTUR_UPPER_EX = {"C": "\u212d", "H": "\u210c", "I": "\u2111", "R": "\u211c", "Z": "\u2128"}

def _build_lower_pool(letter: str, cyrillic_greek: str) -> list[str]:
    """Return all visually-similar alternatives for a lowercase Latin letter."""
    idx = ord(letter) - ord("a")
    pool: list[str] = [cyrillic_greek]
    _MATH_LOWER_BASES = [
        (0x1D41A, None),          # bold
...
def _build_upper_pool(letter: str, cyrillic_greek: str) -> list[str]:
...
HOMOGLYPH_POOLS: dict[str, list[str]] = {
    lc: _build_lower_pool(lc, cg)
    for lc, cg in HOMOGLYPHS.items()
}

HOMOGLYPH_POOLS_UPPER: dict[str, list[str]] = {
